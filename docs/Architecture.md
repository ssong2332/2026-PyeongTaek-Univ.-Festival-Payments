...
| `refund` | cooking → refunded | admin | `refund_channel` 필수 (`cash` / `bank` / `kakaopay` / `toss` — 주문의 결제 경로와 일치해야 함: 현금 주문은 `cash`, 송금 주문은 자기 `transfer_method`) | **필수** | **예** | `closed_at`, `refund_channel` |
...
function resolveTransition(order: { status: OrderStatus; paymentMethod: PaymentMethod; transferMethod: TransferMethod | null },
...
| `orders` | `id` uuid PK · `pickup_number` int NOT NULL UNIQUE · `status` order_status NOT NULL DEFAULT 'pending' · `payment_method` payment_method NOT NULL · `transfer_method` transfer_method NULL · `total_amount` int NOT NULL CHECK ≥0 · `idempotency_key` uuid NOT NULL UNIQUE · `status_token` text NOT NULL UNIQUE · `locale` text NOT NULL DEFAULT 'ko' · `transfer_reported_at` timestamptz NULL · `cancel_requested_at` timestamptz NULL · `cancel_rejected_at` timestamptz NULL · `acknowledged_at` timestamptz NULL · `acknowledged_by` uuid NULL · `paid_at`, `cooking_started_at`, `completed_at`, `closed_at` timestamptz NULL · `refund_channel` refund_channel NULL · `created_at`, `updated_at` timestamptz NOT NULL DEFAULT now() · CHECK `((payment_method='transfer') = (transfer_method IS NOT NULL))` | 인덱스: `(status, created_at)`, `(created_at)`, `(pickup_number)`. `updated_at`은 트리거로 갱신(Realtime 병합 기준) |
...
Enum: `order_status` (위 7개) · `payment_method ('cash','transfer')` · `transfer_method ('bank','kakaopay','toss')` · `refund_channel ('cash','bank','kakaopay','toss')` · `actor_type ('admin','system','customer')`.
...
|---|---|---|
| `create_order` | `(p_idempotency_key uuid, p_payment_method payment_method, p_transfer_method transfer_method, p_locale text, p_items jsonb) RETURNS jsonb` — ADR-0002 | `OUT_OF_STOCK` (detail: menu_item_id, available) · `MENU_UNAVAILABLE` · `INVALID_OPTION` · `EMPTY_ITEMS` |
...
| `GET /api/settings/transfer` | — | `TransferSettingsDto { configured: boolean, bankName, accountNumber, accountHolder, kakaopayUrlTemplate, tossUrlTemplate }` | `transfer.*` 키만. 빈값은 `''`. `configured` = 은행 3값 모두 비어있지 않음 OR 어느 템플릿이든 비어있지 않음(수단별 판단은 클라이언트 `TransferGuide`가 함). 비어 있으면 서버 `warn` 로그(F-44) |
| `POST /api/orders` | `CreateOrderRequest { idempotencyKey: uuid, paymentMethod: 'cash'|'transfer', transferMethod?: 'bank'|'kakaopay'|'toss', locale, items: [{ menuItemId: uuid, quantity: int 1..99, optionIds: uuid[] }] (1..20개) }` — 가격 필드 없음(있어도 zod `strict`로 400) | 201 신규 / 200 멱등 재요청: `CreateOrderResponse { orderId, pickupNumber, statusToken, status, totalAmount, createdAt, created: boolean }` | `transfer`인데 `transferMethod` 없음 또는 `cash`인데 있음 → 400. 멱등키 기존 주문이면 속도 제한 미소비로 200. 신규면 `consume_rate_limit` → 초과 시 429 `RATE_LIMITED`(ADR-0009). 이후 ADR-0002 함수. 에러 409 OUT_OF_STOCK 등 |
| `GET /api/orders/{token}` | 경로 토큰 64 hex | `OrderStatusDto { orderId, pickupNumber, status, paymentMethod, transferMethod, totalAmount, items: [{ name, quantity, options: string[], lineTotal }], createdAt, transferReportedAt, cancelRequestedAt, cancelRejectedAt, aheadCount, canTransferReport, canCancelRequest }` | 토큰 형식 불일치·미존재 모두 404. `aheadCount` = `count_waiting_before(created_at)`. `name`/`options`는 주문 시 `locale` 기준 스냅샷(ko 폴백). 클라이언트 5초 폴링 |
...
| 메서드·경로 | 요청 | 응답 | 규칙 |
|---|---|---|---|
| `GET /api/admin/orders?date=YYYY-MM-DD&status=&pickupNumber=` | `date` 기본 오늘(KST). `status` 콤마 목록 선택. `pickupNumber` 정수(있으면 date 무시) | `{ orders: AdminOrderDto[], unacknowledgedCount }` — `created_at DESC` | `AdminOrderDto { id, pickupNumber, status, paymentMethod, transferMethod, totalAmount, items: [{ menuNameKo, quantity, options: [{ nameKo, extraPrice }], lineTotal }], createdAt, updatedAt, acknowledgedAt, transferReportedAt, cancelRequestedAt, cancelRejectedAt, paidAt, cookingStartedAt, completedAt, closedAt, refundChannel, lastReason, availableActions: TransitionAction[] }`. F-22: `pickupNumber` 결과 0건이면 `orders: []` |
...
| `GET /api/admin/stats/csv?from=YYYY-MM-DD&to=YYYY-MM-DD` | 기본 축제 전체(2026-10-07~08 — `app_settings`가 아니라 요청 기본값 상수 `FESTIVAL_DATES`) | `text/csv; charset=utf-8` + BOM, `Content-Disposition: attachment; filename="orders_{from}_{to}.csv"` | 헤더 14개(DECISIONS #20): `주문 ID,픽업 번호,주문 시각,메뉴,옵션,수량,금액,결제수단,송금 하위 수단,상태,취소/환불 사유,주문 합계,결제확인 시각,완료 시각`. 옵션은 `;`로 연결. 상태·결제수단은 한글 라벨. 0건이면 헤더만 |
...
| 결제수단 선택/확정 | `/checkout` | `useCheckout` — `{ paymentMethod, transferMethod, idempotencyKey, submitting, error, submit }` | `POST /api/orders` (재시도 DECISIONS #24) | 미선택 → 확정 비활성; `submitting` 중 버튼 잠금; `OUT_OF_STOCK` → `details`로 항목 표시 + `/cart` 복귀; 네트워크 실패 → 수동 재시도 버튼(장바구니 유지); `RATE_LIMITED`(429) → `errors.RATE_LIMITED` 문구("잠시 후 다시 시도") + 같은 수동 재시도 버튼 + 장바구니·멱등키 유지(F-47); 성공 → cart·key 폐기 후 `/orders/{token}` |
| 주문 완료/상태 | `/orders/[token]` | `useOrderStatus(token)` — `{ status:'loading'|'ready'|'notFound'|'error', order, actions }` | `GET /api/orders/{token}` 5초 폴링 + `GET /api/settings/transfer`(송금 주문만, 1회) | 404 → `NotFound('order.notFound')`; 로딩 표시; `TransferGuide`(수단별 분기, 템플릿 `{amount}` 유무로 폴백, 설정 빈값 → "준비 중"); `[송금했어요]`/`[취소 요청]` 버튼은 `canTransferReport`/`canCancelRequest`; 클릭 중 잠금, 실패 토스트; 거절됨 문구 `cancelRejectedAt` |
...
| 편집 필드 | ADR-0004 키 8개 전부: `transfer.bank_name`·`transfer.account_number`·`transfer.account_holder`·`transfer.kakaopay_url_template`·`transfer.toss_url_template`(text), `auto_complete.enabled`(토글), `auto_complete.minutes`(int 1..120), `payment.expire_minutes`(int 1..120) |
...
| 빌드·릴리스 파이프라인 | Vercel Git 연동: `main` push → 프로덕션 빌드·배포(자동), feature 브랜치 push → 프리뷰 배포. 빌드 = `npm run build`. 테스트는 GitHub Actions(테스트 전략 "CI" 행)가 push·PR마다 실행하고, 병합 전 DoD가 CI 녹색을 요구한다. Vercel 배포와 CI는 독립(Vercel은 CI 결과를 기다리지 않음) — `dev`·`main` 보호는 GitHub 브랜치 규칙(PR 필수 + `unit-build`·`integration` 필수 체크)으로 강제(T-01에서 설정, 리포 관리자 권한 필요 — 없으면 Open Question) |
...
| 항목 | 결정 |
|---|---|
| 시크릿 | `SUPABASE_SERVICE_ROLE_KEY`·(2차)`PHONE_ENCRYPTION_KEY`는 서버 전용, `NEXT_PUBLIC_` 금지. ESLint `no-restricted-imports`로 `infra/supabase/server.ts`를 클라이언트 컴포넌트(`'use client'`)에서 import 금지(T-01 설정). 계좌·송금 URL은 DB(ADR-0004), 리포지토리 0건 — 리뷰 시 `grep`로 확인 |
