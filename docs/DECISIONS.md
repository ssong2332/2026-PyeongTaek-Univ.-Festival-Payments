...
| 35 | 2026-09-22 | **(폐기 → #43)** (B) 호스팅은 Vercel Hobby로 설계하되 **T-50 약관 확인 전 미확정** — Hobby 상업적 이용 제한(추정)에 해당하면 Vercel Pro 1개월(설계 영향 없음) 또는 Cloudflare Pages 무료(어댑터·IP 헤더·도메인 변경 → 팀장 확인) | 사용자 승인(PRD Open Question #35, N-14 예외). 전환 여부는 사실 확인 사항이라 지금 결정하지 않는다 | — |
...
| 42 | 2026-09-25 | FE2(관리자 화면)를 김 혁(DB2 겸임)에게 이관, 김희진은 FE1(고객 화면) 전담 | 프론트 1인 집중 해소(09-26~10-03 김희진 18칸 → 11칸) | —
| 43 | 2026-09-23 | 호스팅은 **Cloudflare Workers Free + Supabase Free**(비용 0원). Vercel Hobby 미사용 — #35 대체 | T-50 검증 결과 Hobby는 개인·비상업 용도 제한이라 부스 판매에 사용 불가(PRD N-14·Open Question #35, Architecture 배포 절). 2026-09-25 DECISIONS 누락분 정정 | —
| 44 | 2026-09-25 | DECISIONS #36의 주문 생성 속도 제한 기본값을 IP당 분당 5건에서 100건으로 변경. `ORDER_CREATE_RATE_LIMIT` 단일 상수에서 조정하고, 멱등 재요청 예외·429 응답·Postgres 카운터 설계는 유지 | 캠퍼스 공용 IP를 통한 정상 고객 오차단 가능성을 낮추기 위한 사용자 결정. 현장 주문량 관측 후 재조정 | [ADR-0009](adr/0009-order-rate-limit.md) |
