...
| 14 | 2026-09-22 | 카카오페이·토스 송금 URL은 `{amount}` 자리표시자 템플릿. 자리표시자 없으면 고객 화면이 자동 폴백(금액 크게 + 직접 입력 안내) | T-34 검증 결과를 코드가 아닌 데이터로 반영(F-42) | ADR-0004 |
...
| 20 | 2026-09-22 | CSV(F-29)는 **항목당 1행**. 헤더 14개 = PRD 11개 + `주문 합계`·`결제확인 시각`·`완료 시각`. UTF-8 BOM. CSV 매출 = 상태 ∈ {결제확인, 조리중, 완료} 행의 `금액` 합 = F-28 | 옵션·수량이 항목별이라 병합하면 합계 검증이 불가 | — |
