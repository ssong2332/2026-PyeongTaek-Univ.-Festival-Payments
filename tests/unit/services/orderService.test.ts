...
import { createOrder, reportTransfer } from "@/services/orderService";
...
function fakeTransferRepo(result: TransferReportResult) {
...
describe("orderService.reportTransfer", () => {
  it("처음 신고하면 지금 시각으로 기록하고 그 시각을 돌려준다", async () => {
    const { orderRepository, calls } = fakeTransferRepo({ outcome: "reported", transferReportedAt: NOW.toISOString() });
  ...
  it("이미 신고된 주문이면 처음 시각 그대로 돌려준다(연타 멱등)", async () => {
    ...
    const { orderRepository } = fakeTransferRepo({ outcome: "already_reported", transferReportedAt: first });
