...
| P1 프로토타입 1 | 09-25 ~ 09-28 | 핵심 주문 루프(메뉴 → 주문 → 결제 안내 → 관리자 확인 → 고객 상태 확인) + 09-28 프로토타입 배포 |
| P2 프로토타입 2 | 09-29 ~ 10-01 | 실제 운영용 안전장치(취소·환불, 자동 만료, 재고 관리, 속도 제한 등) |
...
임계 경로(P1): T-01 → T-03 → `create_order` 함수 → T-07/T-08 → T-09 → T-31 → 09-28 프로토타입 배포.
