...
축제 방문객이 QR로 접속해 메뉴·옵션을 고르고 현금 또는 송금(계좌이체·카카오페이)으로 주문하고, 부스 운영진이 실시간 대시보드에서 입금 확인·조리·완료를 처리하는 웹 서비스.
...
| 결제 | 현금 · 송금(계좌이체·카카오페이) — 토스는 기존 `toss.me` 종료로 대체 방식 확정 전 비활성, PG 없음, 관리자 수동 확인 |
...
카카오페이는 주문 금액이 자동 입력되지 않으므로 고객 화면에서 주문 금액을 크게 표시하고 같은 금액을 직접 입력하도록 안내한다.

기존 `toss.me` 방식은 사용하지 않으며 공식 대체 방식이 확정되기 전까지 토스 URL은 빈 값으로 유지한다.
